...
proc regress_cov_v {} {
    ...
    vcover report -zeros -details -code bcesft -testHitDataAll -html coverage/IP.ucdb -htmldir coverage/html_report
...
proc run_sv {} {
    ...
    vlog -sv ../rtl/apb_if.sv
    vlog ../rtl/*.v
    
    # 2. Biên dịch các class OOP theo đúng thứ tự (từ tb_sv)
    vlog -sv ../tb_sv/apb_trans.sv
    vlog -sv ../tb_sv/apb_driver.sv
    vlog -sv ../tb_sv/apb_generator.sv
    vlog -sv ../tb_sv/apb_monitor.sv
    vlog -sv ../tb_sv/apb_scoreboard.sv
    vlog -sv ../tb_sv/apb_environment.sv
    
    # 3. Biên dịch Top Testbench
    vlog -sv ../tb_sv/tb_timer_top.sv
    
    echo "--- \[SYSTEMVERILOG FLOW\] Bat dau mo phong ---"
    
    # Chạy mô phỏng (Sử dụng random seed để phát huy sức mạnh OOP)
    vsim -sv_seed random -voptargs="+acc" -onfinish stop work.tb_timer_top
